...
class Mesh:
    ...
    def export_pypan_mesh(self, filename):
        """Exports the mesh with all relevant parameters to make reloading faster.
        This is done in a custom format for PyPan not recognized by other programs,
        so far as we know. Feel free to write a parser.
    ...
    def get_vtk_data(self):
        ...
        # Declare storage
        vertices = []
        panels = [] # First index is the number of vertices, the rest are the vertex indices
        ...
        for panel in self.panels:
            ...
            for vertex in panel.vertices:
                ind = self._check_for_vertex(vertex, vertices)

                # Not in list
                if ind == -1:
                    vertices.append(vertex)
                    panel_info.append(str(i))
                    i += 1

                # In list
                else:
                    panel_info.append(str(ind))
