...
class VortexRingSolver(Solver):
    ...
    def _export_potential(self, filename, **kwargs):
        ...
        ----------
        filename : str
            Filename to export the potential to. Must be ".vtk".

        buffers : list, optional
            Number of times to extend the computational domain in each coordinate direction to either side. Defaults to 1.

        res : list, optional
            Number of points to distribute in the each coordinate direction. Defaults to 200.

        verbose : bool, optional
        """

        # Get kwargs
        buffers = np.array(kwargs.get("buffers", [1.0, 1.0, 1.0]))
        res = kwargs.get("res", [200, 200, 200])
        verbose = kwargs.get("verbose", False)

        # Find bounds for computational mesh
        maxs = np.max(self._mesh.vertices, axis=0)
        mins = np.min(self._mesh.vertices, axis=0)
        deltas = maxs-mins

        # Determine bounding box
        maxs_out = maxs+deltas*buffers
        ...
        with open(filename, 'w') as export_handle:
            ...
            print("PyPan potential file. Generated by PyPan, USU AeroLab (c) 2020.", file=export_handle)
            print("ASCII", file=export_handle)

            # Write dataset
            print("DATASET UNSTRUCTURED_GRID", file=export_handle)
